    ...
    try {
      ...
      try {
        ...
        if (error.response && error.response.data && error.response.data.errors) {
          ...
          await interaction.reply({content: `Invalid data provided: ${JSON.stringify(error.response.data.errors)}`, ephemeral: true});
        } else if (error.response && error.response.status === 429) {
          await interaction.reply({content: 'Rate limit reached. Please try again later.', ephemeral: true});
        } else {
          await interaction.reply({content: 'There was an error creating the user. Please try again later.', ephemeral: true});
        }
      }
    } catch (error) {
      console.error(error);
